...
const Pages = (() => {
  ...
  function beauty(){
    ...
    <div style="margin-top:12px">${data.map((d,i)=>`<div class="vid-card"><div class="vid-thumb"><div class="play"><span>${d.col}</span></div></div><div class="vid-body"><div class="vt">${d.t}</div><div class="vs">${d.s}</div><div class="meta"><span class="pill">${d.src}</span></div><div class="vid-actions">${d.url?`<a class="btn sm" href="${d.url}" target="_blank" rel="noopener">▶ 打开视频</a>`:''}<button class="btn ghost sm" data-bfav="${tab+i}">${fav.includes(tab+i)?'⭐ 已收藏':'☆ 收藏'}</button></div></div></div>`).join('')}</div>`;
    function init(){
