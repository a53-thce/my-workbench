...
const Pages = (() => {
  ...
  function beauty(){
    ...
    <div style="margin-top:12px">${data.map((d,i)=>{const thumb=d.bvid?`<iframe src="${C.biliEmbed(d.bvid)}" allowfullscreen scrolling="no"></iframe>`:`<div class="play"><span>${d.col}</span></div>`;return `<div class="vid-card"><div class="vid-thumb">${thumb}</div><div class="vid-body"><div class="vt">${d.t}</div><div class="vs">${d.s}</div><div class="meta">${d.ref?`<span class="pill">推荐 ${d.ref}</span>`:''}<span class="pill">B站教程</span></div><div class="vid-actions"><a class="btn sm" href="${d.url}" target="_blank" rel="noopener">▶ 打开视频</a><button class="btn ghost sm" data-bfav="${tab+i}">${fav.includes(tab+i)?'⭐ 已收藏':'☆ 收藏'}</button></div></div></div>`;}).join('')}</div>`;
    function init(){
